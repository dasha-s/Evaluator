...
public:
    Evaluator( const string &s ) : str( s ) { // constructor receives an exp
        ...
        PREC_TABLE[6].inputSymbol = 21;     
        PREC_TABLE[6].topOfStack = 22;      //
        PREC_TABLE[7].inputSymbol = 21;
...
private:
    ...
    // Internal routines
    NumericType getTop(char &var );                    // Get top of postfix stack
    void performOp( TokenType topOp );
    void binaryOp( TokenType topOp );         // Process an operator
    void unaryOp( TokenType topOp );         // Process an operator
